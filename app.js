...
for (const el of CALC_DIC) {
  ...
  if (el.type === "number" || el.type === "operator") {
    element.addEventListener("click", () => operation(el.operator));
  }
  if (el.type === "clear") {
...
const operation = (operator) => {
  if (OPERATORS.find((el) => el === operator)) {
    ...
    if (
      ...
      let newCalcDisplay = calculationDisplay.textContent
        .split("")
        .splice(0, calculationDisplay.textContent.length - 1)
        .join("");

      calculationDisplay.textContent = newCalcDisplay + "+";
      yield;
    } else if (
      OPERATORS.find(
        (el) =>
  ...
  if (
    ...
    display.textContent
      .split("")
      .splice(display.textContent.length - 1, 1)
      .join("") === "."
  ) {
    display.textContent = display.textContent.trim() + operator;
  } else if (operator !== "=") {
    display.textContent = operator;
  }

  calculationDisplay.textContent =
    calculationDisplay.textContent.trim() + operator;
};
...
const decimal = () => {
  if (calculationDisplay.textContent === "") {
    return;
  }
